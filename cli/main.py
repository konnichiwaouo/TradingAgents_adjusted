...
# =======================================================
# [新增] 獨立的訊號解析函式 (供 main 和 backtest_runner 共用)
# =======================================================
def parse_signal_from_content(content):
    """
    從文字內容中解析 BUY/SELL/HOLD 訊號。
    支援 JSON 區塊與 Regex 文字搜尋。
    """
    final_signal = "HOLD" # 預設

    try:
        # --- 策略 A: 優先尋找 JSON 區塊 ---
        json_match = re.search(r"```json\s*(\{.*?\})\s*```", content, re.DOTALL)
        if not json_match:
            json_match = re.search(r"(\{.*\"action\":\s*\"(BUY|SELL|HOLD)\".*\})", content, re.DOTALL | re.IGNORECASE)
        ...
        # --- 策略 B: 文字解析 ---
        # 只搜尋結論相關的標題行
        target_lines = re.findall(r"^(?:##|###|\*\*)\s*(?:Recommendation|Conclusion|Decision|Final Thoughts).*?$", content, re.MULTILINE | re.IGNORECASE)
        scan_text = " ".join(target_lines) if target_lines else content[-500:] 
        
        matches = re.findall(r"\b(BUY|SELL|HOLD)\b", scan_text, re.IGNORECASE)
        if matches:
            raw_signal = matches[0].upper()
            idx = scan_text.upper().find(raw_signal)
            context = scan_text[max(0, idx-20):idx].upper()
            
            if "NOT" in context or "AVOID" in context:
                return "HOLD"
            else:
                return raw_signal
                
    except Exception as e:
        print(f"⚠️ 解析訊號發生錯誤: {e}")
    
    return final_signal

def run_analysis_execution(selections):
    """
    Executes the trading analysis based on the provided selections dictionary.
    This function is separated from UI input logic to allow batch execution.
    """
    ...
    if config["llm_provider"] == "azure":
        ...
        console.print(f"[dim]Azure Configured: {config['deep_think_llm']} / {config['embedding_model']}[/dim]")
    ...
    # === [關鍵修正 1]：定義 Decorator 時加入安全檢查 (Safety Check) ===
    def save_message_decorator(obj, func_name):
        func = getattr(obj, func_name)
        @wraps(func)
        def wrapper(*args, **kwargs):
            func(*args, **kwargs)
            # 檢查是否有訊息，避免 index out of range
            if obj.messages: 
    ...
    # === [關鍵修正 2]：先「還原」方法，再「清空」資料 ===
    # 這一步非常重要！避免迴圈跑第二輪時發生 Wrapper 包 Wrapper 的慘劇
    
    # 1. 還原為最原始的 Class Method (Un-decorate)
    message_buffer.add_message = types.MethodType(MessageBuffer.add_message, message_buffer)
    message_buffer.add_tool_call = types.MethodType(MessageBuffer.add_tool_call, message_buffer)
    message_buffer.update_report_section = types.MethodType(MessageBuffer.update_report_section, message_buffer)

    # 2. 清空上一輪的資料 (Reset Data)
    message_buffer.messages.clear()
    message_buffer.tool_calls.clear()
    message_buffer.report_sections = {k: None for k in message_buffer.report_sections}
    message_buffer.current_report = None
    message_buffer.final_report = None
    
    # 3. 重新套用 Decorator (Re-decorate)
    # 現在套用的是乾淨的 log_file 路徑
    ...
    with Live(layout, refresh_per_second=4) as live:
        ...
        if selections['analysts']:
            first_agent_name = analyst_name_map.get(selections['analysts'][0], "Market Analyst")
            message_buffer.update_agent_status(first_agent_name, "in_progress")
        ...
        try:
            for chunk in graph.graph.stream(init_agent_state, **args):
                if len(chunk["messages"]) > 0:
                    ...
                    if "sentiment_report" in chunk and chunk["sentiment_report"]:
                        ...
                        if any(a.value == "news" for a in selections["analysts"]):
                    ...
                    if "news_report" in chunk and chunk["news_report"]:
                        ...
                        if any(a.value == "fundamentals" for a in selections["analysts"]):
            ...
            if 'display_complete_report' in globals():
            ...
            if "final_trade_decision" in final_state:
                final_signal = parse_signal_from_content(final_state["final_trade_decision"])
                print(f"🔍 [AI Output] 最終決策: {final_signal}")
...
def run_analysis():
    ...
    run_analysis_execution(selections)
